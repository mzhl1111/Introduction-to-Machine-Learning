...
plot(1:100,recorder1,1:100,recorder,1:100, ones(1,100)*0.0102, 1:100, ones(1,100)* 0.0774)
xlabel(' Number of trees to learn in the ensemble')
ylabel( 'Out of bag error(cross-validation error )')
legend('3 vs 5 (bagging)','1 vs 3 (bagging)','1 vs 3 (single tree)','3 vs 5 (single tree)')
...
recorder3 = zeros(1,100);
for i = 1:100
    recorder3(i) = BaggedTrees_testErr(X_tr, Y_tr,X_te,Y_te, i);
    if rem(i,20)==0 
        fprintf('=')
    end 
end 

recorder4 = zeros(1,100);
for i = 1:100
    recorder4(i) = BaggedTrees_testErr(X_tr2, Y_tr2,X_te2, Y_te2 , i);
    if rem(i,20)==0 
        fprintf('=')
    end 
end

plot(1:100,recorder4,1:100,recorder3,1:100,ones(1,100)*0.0163,1:100,ones(1,100)*0.1196)
xlabel(' Number of trees to learn in the ensemble')
ylabel( 'test error')
legend('3 vs 5(bagging)','1 vs 3(bagging)','1 vs 3 (single tree)','3 vs 5 (single tree)')
